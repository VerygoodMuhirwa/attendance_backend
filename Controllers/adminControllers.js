...
const registerAdmin = async (req, res) => {
  try {
    ...
    if (userExists)
      return res
        .status(200)
        .send({ error: "The user with that email already exists" });
    ...
    if (!admin)
      return res.status(500).send({error:"Faced an error when registering admin"});
...
const loginAdmin = async (req, res) => {
  try {
    const { email, password } = req.body;
    const userExists = await adminModel.findOne({ email });
    if (!userExists) return res.status(200).json({error:"Invalid password or email"});
